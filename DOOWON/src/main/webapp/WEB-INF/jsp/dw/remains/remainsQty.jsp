...
<div class="flex justify-end">
  <button type="button" onclick="fn_logOut()"
  		  class="text-base font-medium rounded px-2 py-1">
    	로그아웃
  ...
  <form class="w-full shrink-0 grid grid-rows-3 grid-cols-[auto_repeat(5,1fr)_auto_repeat(5,1fr)]
    ...
    <div class="row-start-1 col-span-6 col-end-13 flex flex-row gap-2 items-end justify-end">
      <button type="button" id="btnSearch" onclick="fn_searchRemainsView()" style="background-color: #283593; border: 1px solid #1A237E;"
        	  class="text-white font-medium rounded px-5 py-2 focus:outline-none duration-300 row-start-1">
        <i class="fa-regular fa-search mr-1"></i>
        	검색
      </button>
      <button type="button" onclick="fn_remainsQtyClear()" class="text-blak font-medium rounded px-5 py-2" style="background-color: #e2f3fd; border: 1px solid #1A237E;">
    ...
    <div class="flex flex-row flex-wrap gap-4 col-start-2 col-span-5 row-start-1 items-center *:flex *:flex-row *:items-center *:gap-2 font-medium text-gray-900">
      <label for="default-radio-1">
        <input id="default-radio-1" type="radio" value="01"
          	   ...
          	   class="w-4 h-4 bg-gray-100 border-gray-300 dark:focus:ring-primary-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
        <span>전체</span>
      </label>
      <label for="default-radio-2">
        <input id="default-radio-2" type="radio" value="02"
          	   name="remainsView_srch1" class="w-4 h-4 bg-gray-100 border-gray-300 dark:focus:ring-primary-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
         <span>유</span>
      </label>
    ...
    <select id="remainsDtType" class="row-start-2 col-start-2 col-span-1 bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:border-primary-500 block py-1 px-2.5">
    ...
    <div id="exportView_div1" class="col-start-7 row-start-2 col-span-4 flex items-center gap-1">
      <button id="today-button" type="button" onclick="fn_todayBtn()"
          style="border: 1px solid #1A237E;" onmouseover="this.style.backgroundColor='#e2f3fd'" onmouseout="this.style.backgroundColor='white'"
          class="py-2 px-4 text-sm font-medium text-gray-900 rounded-lg border
          border-primary-200 hover:text-black">
       	  당일
      </button>
    ...
    <input type="text" id="remainsView_srch4" onkeyup="enterkey()" class="row-start-3 col-start-2 col-span-5 bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:border-primary-500 block py-1 px-2.5"
    ...
    <input type="text" id="remainsView_srch5" onkeyup="enterkey()" 
           class="row-start-3 col-start-8 col-span-5 bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:border-primary-500 block px-2.5 py-1"
           placeholder="INVOICE번호를 입력해주세요.">
           
    <select id="remainsSrchType1" style="width:120px;" class="row-start-4 col-start-1 col-span-1 bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:border-primary-500 block px-1 py-1">
    ...
    <input type="text" id="remainsSrchText1"
           class="row-start-4 col-start-2 col-span-3 bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:border-primary-500 block px-2.5 py-1"
      	   onkeyup="enterkey()">
      	   
    <select id="remainsSrchType2" class="row-start-4 col-start-5 col-span-1 bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:border-primary-500 block px-1 py-1">
    	<option value="" selected>조건선택</option>
    ...
    <select id="remainsSrchType3" class="row-start-4 col-span-3 col-start-5.5 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:border-primary-500 block px-2.5 py-1">
    	<option value="" selected></option>
	</select>
	
	<select id="remainsSrchType4" class="row-start-6 col-start-9 col-span-1 bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:border-primary-500 block px-1 py-1">
    	<option value="" selected>조건선택</option>
      	<option value="exp_firm">수출화주</option>
      	<option value="Ta_St_isoNm">목적국</option>
      	<option value="caseType">용기 Type</option>
    ...
    <select id="remainsSrchType5" class="row-start-4 col-span-3 col-start-10 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:border-primary-500 block px-2.5 py-1">
    	<option value="" selected></option>
	</select>
  </form>
  ...
  <div class="flex items-center justify-between mb-3">
    ...
    <div class="flex items-center gap-2">
      <button type="button" onclick="fn_remainsViewExcelDown()" 
        class="text-white font-medium rounded px-2.5 py-1.5 row-start-1 text-base"
        style="background-color: #283593; border: 1px solid #1A237E;">
        <i class="fa-regular fa-download"></i>Excel 다운로드
      </button>
      <select id="remainsViewPageRow" name="remainsViewPageRow" style="background-color: #e2f3fd; border: 1px solid #1A237E;"
        class="w-36 h-9 text-gray bg-primary-100 hover:bg-primary-200 font-medium rounded text-base px-2.5 py-1.5 text-center inline-flex items-center border-primary-500 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
...
<div id="transInfo-container" class="mx-auto p-3 border border-gray-300 w-full" style="height: 230px;">
	<div class="flex items-center justify-between w-full shrink-0">
    	<div class="flex items-center gap-4">
    		<p class="card-title inline"><i class="fa-duotone fa-chart-network text-primary-900"></i>&nbsp;
	    		잔량 소진 이력<!-- : <span id="tradeGapLanPageCnt">0</span> -->
	    	</p>
      		<label for="default-radio-6" style="margin-left:20px;">
      			<input id="default-radio-6" type="radio" name="remainsInputType" value="read" checked class="w-4 h-4 bg-gray-100 border-gray-300 dark:focus:ring-primary-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
      			<span>읽기</span>
      		</label>
      		<label for="default-radio-7" style="margin-left:20px;">
      			<input id="default-radio-7" type="radio" name="remainsInputType" value="enrol" class="w-4 h-4 bg-gray-100 border-gray-300 dark:focus:ring-primary-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
      			<span>등록</span>
      		</label>
    	...
    	<div class="flex items-center gap-2 ml-auto">
		    <button type="button" id="remainsInputSave" onclick="fn_remainsInputSave()" style="background-color: #1A237E; border: 1px solid #1A237E;"
		      class="text-white rounded px-2.5 py-1.5 row-start-1 text-base">
		      <i class="fa-regular fa-floppy-disk"></i>
		      	저장
		    </button>
		    <button type="button" id="remainsInputDel" onclick="fn_remainsInputDel()" style="background-color: #1A237E; border: 1px solid #1A237E;"
		      class="text-white rounded px-2.5 py-1.5 row-start-1 text-base">
